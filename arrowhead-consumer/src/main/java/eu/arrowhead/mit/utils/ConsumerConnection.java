...
public class ConsumerConnection {
	...
	public ResponseEntity<String> getTemperature(String scheme, SystemResponseDTO provider, String uri)
			throws IOException {
	...
	public String getCLTCArrayService(int runs, int currentRun) throws IOException {
		String retEntity = "";
		Properties prop = getProp();
		String securityMode = prop.getProperty(MITConstants.SECURITY_MODE);
		String scheme = "";
		...
		OrchestrationResponseDTO orchestrationResponse = getProducer(scheme, prop,
				MITConstants.MIT_PRODUCER_SERVICE_GET_ARRAY);
		
		// HACK to get correct URI
		String uri = orchestrationResponse.getResponse().get(0).getServiceUri() + "/" + runs + "/" + currentRun;

		if (!orchestrationResponse.getResponse().isEmpty()) {
			retEntity = getTemperature(scheme, orchestrationResponse.getResponse().get(0).getProvider(), uri).getBody();
		}
		return retEntity;
	}
